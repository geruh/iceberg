...
public class TestVariantShredding extends CatalogTestBase {
  ...
  @TestTemplate
  public void testVariantColumnPruningReturnsCorrect() throws IOException {
    spark.conf().set(SparkSQLProperties.SHRED_VARIANTS, "true");
    String values =
        "(1, parse_json('{\"name\": \"Alice\", \"city\": \"NYC\", \"zip\": 10001}')), "
            + "(2, parse_json('{\"name\": \"Bob\", \"city\": \"LA\", \"zip\": 90001}'))";
    sql("INSERT INTO %s VALUES %s", tableName, values);

    // this should trigger column pruning and only the name shredded column should be read
    List<Object[]> result =
        sql(
            "SELECT id, variant_get(address, '$.name', 'string') as name FROM %s ORDER BY id",
            tableName);
  ...
  // TODO: fix nested
//  @TestTemplate
//  public void testVariantColumnPruningWithNestedPath() throws IOException {
//    // Write a shredded variant with nested structure
//    spark.conf().set(SparkSQLProperties.SHRED_VARIANTS, "true");
//    String values =
//        "(1, parse_json('{\"person\": {\"name\": \"Alice\", \"age\": 30}, \"location\": \"NYC\"}')), "
//            + "(2, parse_json('{\"person\": {\"name\": \"Bob\", \"age\": 25}, \"location\": \"LA\"}'))";
//    sql("INSERT INTO %s VALUES %s", tableName, values);
//
//    // Query nested path - should read 'person.name' but not 'person.age' or 'location'
//    List<Object[]> result =
//        sql(
//            "SELECT id, variant_get(address, '$.person.name', 'string') as name FROM %s ORDER BY id",
//            tableName);
//
//    assertThat(result).hasSize(2);
//    assertThat(result.get(0)[0]).isEqualTo(1);
...
//  }
  ...
  @TestTemplate
  public void testColumnPruningBenchmark() throws IOException {
    spark.conf().set(SparkSQLProperties.SHRED_VARIANTS, "true");
    ...
    for (int i = 0; i < 20; i++) {
      ...
      jsonTemplate.append(String.format("\"field%d\": \"value%d\"", i, i));
    }
    jsonTemplate.append("}");
    String jsonStr = jsonTemplate.toString();
    ...
    for (int i = 0; i < numRows; i++) {
      ...
      values.append(String.format("(%d, parse_json('%s'))", i, jsonStr));
    }
    ...
    try (CloseableIterable<FileScanTask> tasks = table.newScan().planFiles()) {
      ...
      try (ParquetFileReader reader = ParquetFileReader.open(inputFile)) {
        ...
        GroupType addressGroup = actualSchema.getType("address").asGroupType();
        assertThat(ParquetSchemaUtil.hasField(addressGroup, "typed_value"))
            .isTrue();
      }
    }

    sql("SELECT id, variant_get(address, '$.field0', 'string') FROM %s", tableName);
    sql("SELECT id, address FROM %s", tableName);

    int iterations = 5;
    long prunedTotalTime = 0;
    for (int i = 0; i < iterations; i++) {
      // spark.catalog().clearCache();

      long start = System.nanoTime();
      List<Object[]> prunedResult =
          sql("SELECT id, variant_get(address, '$.field0', 'string') FROM %s", tableName);
      long end = System.nanoTime();
      prunedTotalTime += (end - start);

      assertThat(prunedResult).hasSize(numRows);
    }

    // Query reading full varinat
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      long start = System.nanoTime();
      List<Object[]> fullResult = sql("SELECT id, address FROM %s", tableName);
      long end = System.nanoTime();
      fullTotalTime += (end - start);

      assertThat(fullResult).hasSize(numRows);
    }

    double prunedAvgMs = ((double) prunedTotalTime / iterations) / 1_000_000.0;
    double fullAvgMs = ((double) fullTotalTime / iterations) / 1_000_000.0;
    double speedup = fullAvgMs / prunedAvgMs;

    System.out.println("BENCH");
    System.out.println(String.format("Pruned read:   %.2f ms avg", prunedAvgMs));
    System.out.println(String.format("Full read:   %.2f ms avg", fullAvgMs));
