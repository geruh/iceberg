...
public class SparkSchemaUtil {
  ...
  /**
   * Convert a list of Spark columns to an Iceberg {@link Schema} with default values.
   *
   * @param columns a list of Spark columns
   * @return the equivalent Iceberg Schema with default values
   */
  ...
  static Literal<?> convertSparkDefaultValueToLiteral(
    ...
    // Reject all complex and unsupported types as Iceberg Literals only support primitives
    ...
    if (typeId == Type.TypeID.STRUCT
      ...
      throw new UnsupportedOperationException(
          String.format(
              "Adding column %s with unsupported type: %s and default value is not supported.",
              fieldName, type.typeId().name()));
    }
